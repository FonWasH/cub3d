...
/*   Updated: 2025/04/19 03:23:07 by juperez          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	show_fps(float delta_time, void *mlx_ptr, void *win_ptr)
{
	...
	if (current_fps >= 1.0f)
	{
		...
		mlx_string_put(mlx_ptr, win_ptr, 10, 20, WHITE, s_int);
	}
	else
	{
		fps_dec_str(current_fps, s_dec);
		mlx_string_put(mlx_ptr, win_ptr, 10, 20, WHITE, "  0");
		mlx_string_put(mlx_ptr, win_ptr, 28, 20, WHITE, ".");
		mlx_string_put(mlx_ptr, win_ptr, 34, 20, WHITE, s_dec);
	}
}
